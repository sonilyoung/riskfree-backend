    ...
    protected MultipartParsingResult parseFileItems(List fileItems, String encoding) {
        ...
        for (Iterator<?> it = fileItems.iterator(); it.hasNext();) {
            ...
            if (fileItem.isFormField()) {
                ...
                if (fileItem.getSize() >= 0 && !StringUtils.isEmpty(fileItem.getName())) {
                    ...
                    System.out.println("::::ATTACH NAME : ]" + fileItem.getName() + "[");

                    bExist = false;
                    for (String att : attList) {
                        if (fileItem.getName().equals(att)) {
                            bExist = true;
                            break;
                        }
                    }
                    ...
                    if (!bExist) {
                        attList.add(fileItem.getName());

                        if (multipartFiles.put(fileItem.getName(), fileList) != null) { // CHANGED!!
                            throw new MultipartException(
                                    "Multiple files for field name [" + file.getName()
