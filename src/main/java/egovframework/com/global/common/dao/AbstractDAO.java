 ...
 * org.springframework.orm.ibatis.support.SqlMapClientDaoSupport을 extends 하고 있으며 CRUD 와 관련한 대표적인
 * method 를 간단하게 호출할 수 있도록 Wrapping 하고 있어 사용자 DAO에서 iBatis sqlMapClient 호출을 쉽게 하며 Bean 생성 시
 * Annotation 기반으로 sqlMapClient 을 쉽게 injection 할 수 있는 공통 로직을 포함하고 있다.
 * </p>
 * 
 * @author 실행환경 개발팀 우병훈
 * @author Vincent Han
...
public abstract class AbstractDAO extends SqlMapClientDaoSupport {
    ...
    protected AbstractDAO() {
        // PMD abstract Rule
        // - If the class is intended to be used as a base class only (not to be instantiated
        // directly)
        // a protected constructor can be provided prevent direct instantiation
    }

    /**
     * Annotation 형식으로 sqlMapClient 를 받아와 이를 super(SqlMapClientDaoSupport) 의 setSqlMapClient 메서드를
     * 호출하여 설정해 준다.
     *
     * @param sqlMapClient - ibatis 의 SQL Map 과의 상호작용을 위한 기본 클래스로 mapped statements(select, insert,
     *        update, delete 등)의 실행을 지원함.
     */
    /*
     * @Resource(name = "sqlMapClient") public void setSuperSqlMapClient(SqlMapClient sqlMapClient)
    ...
    /**
     * 수정 처리 SQL mapping 을 실행한다. 반환값이 없는 대신 예상 결과행 수와 맞지 않으면 예외 오류 발생한다.
     *
     * @param queryId - 수정 처리 SQL mapping 쿼리 ID
     * @param parameterObject - 수정 처리 SQL mapping 입력 데이터(key 조건 및 변경 데이터)를 세팅한 파라메터 객체(보통 VO 또는 Map)
    ...
    /**
     * 삭제 처리 SQL mapping 을 실행한다. 반환값이 없는 대신 예상 결과행 수와 맞지 않으면 예외 오류 발생한다.
     *
     * @param queryId - 삭제 처리 SQL mapping 쿼리 ID
     * @param parameterObject - 삭제 처리 SQL mapping 입력 데이터(일반적으로 key 조건)를 세팅한 파라메터 객체(보통 VO 또는 Map)
     ...
     * @return 결과 List 객체 - SQL mapping 파일에서 지정한 resultClass/resultMap 에 의한 결과 객체(보통 VO 또는 Map)의
     *         List
     */
     ...
     *         List
     */
    public List<?> list(String queryId, final Object parameterObject, int skipResults,
            int maxResults) {
        return getSqlMapClientTemplate().queryForList(queryId, parameterObject, skipResults,
                maxResults);
    }
     ...
     * @return 부분 범위 결과 List 객체 - SQL mapping 파일에서 지정한 resultClass/resultMap 에 의한 부분 범위 결과 객체(보통 VO
     *         또는 Map) List
     */
    public List<?> listWithPaging(String queryId, Object parameterObject, int pageIndex,
            int pageSize) {
     ...
     */
    public Map<?, ?> map(final String queryId, final Object parameterObject,
