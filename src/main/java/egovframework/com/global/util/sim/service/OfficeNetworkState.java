/**
 * Class Name : OfficeNetworkState.java Description : 네트워크(Network)상태 체크 Business Interface class
 ...
 * @since 2009. 02. 02
 * @version 1.0
 * @see The type com.sun.star.lang.XeventListener cannot be resolved. It is indirectly referenced
 *      from required .class files Copyright (C) 2009 by All right reserved.
 */
...
public class OfficeNetworkState {
    ...
    // Log
    // protected static final Log log = LogFactory.getLog(OfficeNetworkState.class);
    ...
    public static String getMyMACAddress(String localIP) {
        ...
        try {
            if ("WINDOWS".equals(Globals.OS_TYPE)) {
                ...
                mac = out.substring(out.indexOf("MAC Address = ") + 14,
                        out.indexOf("MAC Address = ") + 31);
    ...
    public static List<String> getMyPortScan() {
        ...
        try {
            ...
            if ("WINDOWS".equals(Globals.OS_TYPE)) {
                ...
                String[] command =
                        {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR), "SCAN"};
            ...
            ResourceCloseHelper.close(input);
        }
    ...
    public static String getMyIPaddress() {
        try {
            ...
            if (!WebUtil.isIPAddress(InetAddress.getLocalHost().getHostAddress())) {
                throw new RuntimeException(
                        "IP is needed. (" + InetAddress.getLocalHost().getHostAddress() + ")");
            }
    ...
    public static boolean getPingTest(String requestIP) throws Exception {
        ...
        if (!WebUtil.isIPAddress(requestIP)) {
    ...
    public static String getNetWorkInfo(String stringOne) throws IOException {
        ...
        try {
            String cmdStr = GlobalsProperties.getPathProperty(Globals.SERVER_CONF_PATH,
                    "SHELL." + Globals.OS_TYPE + ".getNetWorkInfo");
            String[] command =
                    {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR), stringOne};
            p = Runtime.getRuntime().exec(command);
