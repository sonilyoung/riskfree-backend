 ...
 *  @see
 *
 *  Copyright (C) 2009 by   All right reserved.
 */
...
public class OfficeFileMntrg extends Thread {
	 ...
	 * @param filename
	 */
	protected OfficeFileMntrg(String filename, File logFile) {
		//log.debug("FileMntrg start");
	...
	protected void checkAndConfigure() {
		...
		try {
			...
			*/
			...
			BasicLogger.debug("Checking error", e);
		}
	...
	public void run() {
		while (!interrupted) {
			try {
				...
				BasicLogger.ignore("Interrupted Exception", e);
			}
			checkAndConfigure();
		}
		if (interrupted) {
	...
	public boolean writeLog(String logStr) {
		...
		try {
			...
			ResourceCloseHelper.close(br, bWriter, fWriter);
		}
	...
	public boolean isEnd() {
		...
		try {
			...
			ResourceCloseHelper.close(br, fr);
		}
		return isEnd;
	}
}
