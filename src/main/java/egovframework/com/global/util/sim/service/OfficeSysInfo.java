/**
 * Class Name : OfficeSysInfo.java Description : 시스템정보를 조회하는 Business Interface class Modification
 * Information
 *
...
public class OfficeSysInfo {
    ...
    // Log
    // protected static final Log log = LogFactory.getLog(OfficeSysInfo.class);
    ...
    public static Vector<Map<String, String>> getPrductList() throws Exception {
        ...
        String strlist = GlobalsProperties.getProperty(Globals.SERVER_CONF_PATH, "SERVER_LIST");
    ...
    public static String getPrductVersion(String server) throws Exception {
        ...
        String version = GlobalsProperties.getProperty(Globals.SERVER_CONF_PATH,
                server.toUpperCase() + ".VERSION");
    ...
    public static String getPrductPort(String server) throws Exception {
        ...
        String port = GlobalsProperties.getProperty(Globals.SERVER_CONF_PATH,
                server.toUpperCase() + ".PORT");
    ...
    public static String getPrductStatus(String port) throws Exception {
        ...
        try {
            String cmdStr = GlobalsProperties.getPathProperty(Globals.SERVER_CONF_PATH,
                    "SHELL." + Globals.OS_TYPE + ".getPrductStatus");
            String[] command =
                    {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR), port};
            ...
            if ("UNIX".equals(Globals.OS_TYPE)) {
                ...
                while (true) {
                    ...
                    if (str == null)
                        break;
                    if (str != null && !"".equals(str) && str.indexOf(port) != -1
                            && str.indexOf("LISTENING") != -1 && str.length() <= MAX_STR_LEN) {
                        status = "LISTENING";
                    }
                }
            }
    ...
    public static String getOSName() throws Exception {
        ...
        try {
            String cmdStr = GlobalsProperties.getPathProperty(Globals.SERVER_CONF_PATH,
                    "SHELL." + Globals.OS_TYPE + ".getOSInfo");
            String[] command =
                    {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR), "NAME"};
            p = Runtime.getRuntime().exec(command);
            // p.waitFor();
            ...
            if ("UNIX".equals(Globals.OS_TYPE)) {
                ...
                while (true) {
                    ...
                    if (str == null)
                        break;
                    if (str != null && !"".equals(str) && str.indexOf("OS 이름:") != -1
                            && str.length() <= MAX_STR_LEN) {
        ...
        try {
            ...
            if ("UNIX".equals(Globals.OS_TYPE)) {
                ...
                while (true) {
                    ...
                    if (str == null)
                        break;
                    if (str != null && !"".equals(str) && str.indexOf("OS 버전:") != -1
                            && str.length() <= MAX_STR_LEN) {
    ...
    public static String getOSPrductor() throws Exception {
        ...
        try {
            String cmdStr = GlobalsProperties.getPathProperty(Globals.SERVER_CONF_PATH,
                    "SHELL." + Globals.OS_TYPE + ".getOSInfo");
            String[] command =
                    {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR), "PRDUCTOR"};
            p = Runtime.getRuntime().exec(command);
            // p.waitFor();
            ...
            if ("UNIX".equals(Globals.OS_TYPE)) {
                ...
                while (true) {
                    ...
                    if (str == null)
                        break;
                    if (str != null && !"".equals(str) && str.indexOf("OS 제조업체:") != -1
                            && str.length() <= MAX_STR_LEN) {
        ...
        try {
            ...
            if ("UNIX".equals(Globals.OS_TYPE)) {
                ...
                while (true) {
                    ...
                    if (str == null)
                        break;
                    if (str != null && !"".equals(str) && str.indexOf("프로세서:") != -1
                            && str.length() <= MAX_STR_LEN) {
    ...
    public static List<String> getDiskName() throws Exception {
        ...
        try {
            String cmdStr = GlobalsProperties.getPathProperty(Globals.SERVER_CONF_PATH,
                    "SHELL." + Globals.OS_TYPE + ".getDiskInfo");
            String[] command =
                    {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR), "NAME"};
            p = Runtime.getRuntime().exec(command);
            // p.waitFor();
    ...
    public static float getMoryFullCpcty() throws Exception {
        ...
        /*
         * // 메모리 전체 용량 float cpcty = 0;
         * 
         * Process p = null; try { String cmdStr =
         * GlobalsProperties.getPathProperty(Globals.SERVER_CONF_PATH,
         * "SHELL."+Globals.OS_TYPE+".getMoryInfo"); String[] command = {cmdStr.replace('\\',
         * FILE_SEPARATOR).replace('/', FILE_SEPARATOR), "FULL"}; p =
         * Runtime.getRuntime().exec(command); //p.waitFor();
         * 
         * boolean result = false; BufferedReader b_out = new BufferedReader(new
         * InputStreamReader(p.getInputStream()));
         * 
         * if ("UNIX".equals(Globals.OS_TYPE)) { while (true){ String str = b_out.readLine(); if
         * (str == null) break; if (str != null && !"".equals(str) && str.length() <= MAX_STR_LEN) {
         * str = str.replaceAll("mem=", "").replaceAll(" ", "").replaceAll("MB", "").replaceAll(",",
         * ""); cpcty = Float.parseFloat(str); result = true; } } } else if
         * ("WINDOWS".equals(Globals.OS_TYPE)) { // 메모리 전체용량 = 사용중 용량 + 유효 용량 cpcty =
         * getMoryUsedCpcty() + getMoryFreeCpcty(); }
         * 
         * b_out.close();
         * 
         * }catch(Exception ex){ ex.printStackTrace(); }finally{ if (p != null) p.destroy(); }
         * 
         * return cpcty;
         */
    }
    ...
    public static float getMoryUsedCpcty() throws Exception {
        ...
        try {
            ...
            String[] command =
                    {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR), "USED"};
            p = Runtime.getRuntime().exec(command);
            // p.waitFor();
            ...
            if ("UNIX".equals(Globals.OS_TYPE)) {
                ...
                while (true) {
                    ...
                    if (str == null)
                        break;
                    if (str != null && !"".equals(str) && str.indexOf("총 실제 메모리:") != -1
                            && str.length() <= MAX_STR_LEN) {
                        str = str.replaceAll("총 실제 메모리:", "").replaceAll(" ", "")
                                .replaceAll("MB", "").replaceAll(",", "");
                        fullcpcty = Long.parseLong(str);
                    }
                    if (str != null && !"".equals(str) && str.indexOf("사용 가능한 실제 메모리:") != -1) {
                        str = str.replaceAll("사용 가능한 실제 메모리:", "").replaceAll(" ", "")
                                .replaceAll("MB", "").replaceAll(",", "");
    ...
    public static float getMoryFreeCpcty() throws Exception {
        ...
        try {
            String cmdStr = GlobalsProperties.getPathProperty(Globals.SERVER_CONF_PATH,
                    "SHELL." + Globals.OS_TYPE + ".getMoryInfo");
            String[] command =
                    {cmdStr.replace('\\', FILE_SEPARATOR).replace('/', FILE_SEPARATOR), "FREE"};
            p = Runtime.getRuntime().exec(command);
            // p.waitFor();
     ...
     * @exception Exception
     */
    public static List<String> executeProgram(String propertyKeyword, String[] command)
            throws Exception {
        ...
        try {
            Process p = null;
            String cmdStr = GlobalsProperties.getPathProperty(Globals.SHELL_FILE_PATH,
                    "SHELL." + Globals.OS_TYPE + "." + propertyKeyword);
            command[0] = cmdStr;
            p = Runtime.getRuntime().exec(command);
            p.waitFor();
            // 프로세스 에러시 종료
    ...
    public static List<String[]> getProcessId() {
        ...
        try {
            ...
            if ("WINDOWS".equals(Globals.OS_TYPE)) {
                 ...
                 * log.debug(tmp); command [0] = tmp; p = Runtime.getRuntime().exec(tmp); separator
                 * = " ";
                 */
                separator = " ";
        ...
        try {
            ...
            if ("WINDOWS".equals(Globals.OS_TYPE)) {
                p = Runtime.getRuntime()
                        .exec("tasklist /fo csv /nh /fi \"imagename eq " + processName + "*\"");
                separator = "\",\"";
                ...
                // tmp = "ps -eo \"%p %G %U %c %a\"|awk -F\" \" '{print $1,$2,$3,$4,$5}'|grep
                // "+processName;
                // command [0] = tmp;
