/**
 * Class Name : ClntInfo.java Description : 클라이언트(Client)의 IP주소, OS정보, 웹브라우저정보를 조회하는 Business
...
public class OfficeClntInfo {
    ...
    public static String getClntOsInfo(HttpServletRequest request) throws Exception {
        ...
        String os_conf = GlobalsProperties.getProperty(Globals.CLIENT_CONF_PATH,
                os_info.replaceAll(" ", ""));
        String osInfo = "";
    ...
    public static String getClntWebVer(HttpServletRequest request) throws Exception {
        ...
        for (int i = 0; i < arr.length; i++) {
            ...
            if (s_loc != -1) {
                ...
                webVer = webVer.replaceAll("/", "").replaceAll(";", "").replaceAll("^", "")
