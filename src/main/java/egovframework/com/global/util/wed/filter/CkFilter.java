...
public class CkFilter implements Filter {
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        ckImageSaver = new CkImageSaver(imageDomain, allowFileTypeArr, saveManagerClass);
    ...
    public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
        ...
        if (request.getContentType() == null
            ...
            System.out.println("before CkFilter...");
            chain.doFilter(request, response);
            System.out.println("after CkFilter...");
        } else {
            String app = request.getParameter("app");
            if (app.equals(GlobalsProperties.getProperty("appCode.e-approval"))
                    || app.equals(GlobalsProperties.getProperty("appCode.bbs"))) {
                // encode url
                ckImageSaver.returnEncodeDataToClient(request, response);
            } else {
                // download url
